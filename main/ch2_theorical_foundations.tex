...
\item \textbf{Backpropagation} is the minimization step of the loss function. Once the forward pass is carried out and the error is ontanined the loss-gradient is recursively computed with respect to the given input. Afterwards, it is propagated backwards and the weights of each neuron are updated. Figure \ref{fig:BPim1} depicts the process. Forwards pass, error computation, loss gradient and backwardpropagation is called epoch and compresses each indivual process
